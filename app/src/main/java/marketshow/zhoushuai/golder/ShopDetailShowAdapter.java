...
//详细商品展示适配器
public abstract class ShopDetailShowAdapter extends BaseAdapter implements PinnedHeaderListView.PinnedSectionedHeaderAdapter {

    private static int HEADER_VIEW_TYPE = 0;
    private static int ITEM_VIEW_TYPE = 0;//设置默认为0
    private SparseArray<Integer> mSectionPositionCache; //缓存当前选中位置
    private SparseArray<Integer> mSectionCache;//计算当前选中的值
    private SparseArray<Integer> mSectionCountCache;//计算选中数目
    private int mCount; //保存总数
    private int mSectionCount; //当前选中的数目

     //构造方法初始化值
    public ShopDetailShowAdapter() {
        super();
 ...
 //更新数据
    ...
    public void notifyDataSetChanged() {
        mSectionCache.clear();//清楚当前的缓存
        mSectionPositionCache.clear();//清楚当前选中的位置
        mSectionCountCache.clear();//清楚缓存睡
        mCount = -1;
        mSectionCount = -1;
